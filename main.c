...
#define GATE_CLOSED_STOPED (gatestate == 6)
...
void FC_Check(){  
    ...
    if (!IO.FC_OPEN  && IO.FC_CLOSED) //Gate Closed
        gatestate = CLOSED;
        
    if ((!IO.FC_OPEN  || !IO.FC_CLOSED) && SYS.FIRST_START) //First Start and unknown position
        gatestate = STARTING;
...
void FSM_Main(){
    switch (state){
        ...
        case 1: // Check Gate state  
            if (GATE_CLOSED || GATE_CLOSED_STOPED ) 
                state = 2;
            else
                state = 3;
            break;
